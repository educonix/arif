...
export const ResearchManager = () => {
  const [entries, setEntries] = useState<ResearchPaper[]>([]);
  const [loading, setLoading] = useState(true);
  const [editingEntry, setEditingEntry] = useState<ResearchPaper | null>(null);
  ...
  const handleSave = async (entry: ResearchPaper) => {
    setLoading(true);
    try {
      const submissionData = {
        ...
        youtube_url: processYoutubeUrl(entry.youtube_url),
      ...
      alert('Error saving research paper: ' + (error.message || 'Unknown error'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id: any) => {
    if (confirm('Are you sure you want to delete this research paper?')) {
      ...
      try {
        ...
        if (post?.cover_image && post.cover_image.includes('research_cover')) {
          try {
        ...
        alert('Error deleting research paper: ' + error.message);
      } finally {
        setLoading(false);
      }
    }
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>, entry: ResearchPaper, setEntry: (e: ResearchPaper) => void) => {
  ...
  const renderForm = (entry: ResearchPaper, setEntry: (e: ResearchPaper) => void, onCancel: () => void) => (
    <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 space-y-6">
      <div className="flex justify-between items-center border-b border-slate-100 pb-4">
        <h3 className="text-xl font-bold text-slate-800">{entry.id ? 'Edit Research Paper' : 'Add New Research Paper'}</h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-semibold text-slate-700 mb-1">Authors</label>
            <input
              type="text"
              value={entry.authors}
              onChange={(e) => setEntry({ ...entry, authors: e.target.value })}
              className="w-full border border-slate-200 rounded-xl p-3 focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 outline-none transition-all"
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => setEntry({ ...entry, keywords: e.target.value })}
              className="w-full border border-slate-200 rounded-xl p-3 focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 outline-none transition-all"
        ...
        <div className="md:col-span-2 space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-semibold text-slate-700 mb-1">PDF URL (Google Drive link)</label>
              <input
                type="text"
                value={entry.pdf_url || ''}
                onChange={(e) => setEntry({ ...entry, pdf_url: e.target.value })}
                className="w-full border border-slate-200 rounded-xl p-3 focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 outline-none transition-all"
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setEntry({ ...entry, youtube_url: e.target.value })}
                className="w-full border border-slate-200 rounded-xl p-3 focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 outline-none transition-all"
  ...
  if (loading && !entries.length) {
    return <div className="p-8 text-center text-slate-500">Loading research papers...</div>;
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-bold text-slate-800">Research Management</h2>
          <p className="text-slate-500 mt-1">Manage your research papers and publications.</p>
        </div>
        {!editingEntry && (
          <button
            onClick={() => setEditingEntry({
              title: '', slug: '', subtitle: '', abstract: '', content: '', cover_image: '', pdf_url: '', authors: '', institution: '', keywords: '', youtube_url: '',
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200 text-slate-500 text-sm">
                  ...
                  <th className="p-4 font-semibold">Authors</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {entries.map((entry) => (
                  <tr key={entry.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="p-4 text-slate-600">{entry.authors}</td>
                ...
                {entries.length === 0 && (
                  <tr>
                    <td colSpan={7} className="p-8 text-center text-slate-500">
                      No research papers found. Click "Add Research" to create one.
                    </td>
                  </tr>
                )}
              </tbody>
